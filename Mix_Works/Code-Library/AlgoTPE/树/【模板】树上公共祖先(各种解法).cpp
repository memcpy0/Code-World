...
请你设计并实现 getKthAncestor(int node, int k) 函数，函数返回节点 node 的第 k 个
祖先节点。如果不存在这样的祖先节点，返回 -1 。
...
treeAncestor.getKthAncestor(6, 3);  // 返回 -1 因为不存在满足要求的祖先节点
 ...
 public:
     ...
     TreeAncestor(int n, vector<int>& parent) {
         ...
         for(int j = 1; (1 << j) < n; j++) //先处理每个节点第2^1级祖先,然后处理第2^2级祖先,...直到处理完2^Log2(n)级祖先
             for(int i = 0; i < n; i++)
                 if(p[i][j-1] != -1)
                     p[i][j] = p[p[i][j-1]][j-1];
     } 
     int getKthAncestor(int node, int k) {
         ...
         int z = log2(k);
         return getKthAncestor(p[node][z], k - (1 << z));
     }
 };
...
public: //n个节点从0~n-1
    TreeAncestor(int n, vector<int>& parent) {
        ...
        for (int i = 0; i < n; ++i) { //树的根节点是编号为 0 的节点,顺序处理下来的,都可以 
            for (int j = 1; j <= Log2[depth[i]]; ++j) //只用处理Log2[level]级祖先
...
int lca(int a, int b) {
	...
	while (depth[a] != depth[b]) //查询b的depth[b]-depth[a]级祖先 
		b = father[b][Log2[depth[b] - depth[a]]]; //精妙！！！ 
	if (a == b) return a; 

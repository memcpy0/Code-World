...
**小技巧：在完成依次跳转后，可以使用 `Ctrl+o` 快速回到上一次(跳转前)光标所在位置**，这个技巧很实用，比如在写代码时，忽然想起有个bug，需要修改，这时候跳过去改好了，只需要按下 `Ctrl+o` 就可以回到你之前的位置。`vim` 中会用很多类似的小技巧就等着你去发掘。
...
打开文件，进入普通模式练习上述命令，这会儿你就可以随意  `yy` 了。
```bash
$ vim protocols
 ```

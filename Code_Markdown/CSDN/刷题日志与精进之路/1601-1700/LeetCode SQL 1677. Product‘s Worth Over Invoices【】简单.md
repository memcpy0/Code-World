...
Return the result table ordered by `product_name`. The query result format is in the following example:
...
```
题意：写一个 SQL 查询，对所有产品，返回每类产品的产品名称、剩下的总金额、已付的总金额、取消支付的总金额、退还的总金额。返回结果按照 `product_name` 排序。
